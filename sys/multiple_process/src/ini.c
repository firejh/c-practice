...
int config_btree_insert_section_node(CONFIG_BTREE *config, const char *section_name,
            const char *key, int key_len, void *data, int data_len)
{
    CONFIG_BTREE *section;
    CONFIG_BTREE_NODE *node;
    node = config_btree_find_node(config, section_name);
...
int config_btree_inorder_traverse(CONFIG_BTREE_NODE *node, CONFIG_BTREE_TRAVERSE_CB callback)
{
    ...
    if (!config_btree_inorder_traverse(node->left, callback)) {
        return 0;
    }
    if (!callback(node)) {
        return 0;
    }
    if (!config_btree_inorder_traverse(node->right, callback)) {
        return 0;
    }
...
int config_btree_inorder_save_traverse(CONFIG_BTREE_NODE *node, FILE *fp, CONFIG_BTREE_SAVE_TRAVERSE_CB callback)
{
    ...
    if (!config_btree_inorder_save_traverse(node->left, fp, callback)) {
        return 0;
    }
    ...
    if (!config_btree_inorder_save_traverse(node->right, fp, callback)) {
        return 0;
    }
...
int config_btree_preorder_traverse(CONFIG_BTREE_NODE *node, CONFIG_BTREE_TRAVERSE_CB callback)
{
    ...
    if (!config_btree_preorder_traverse(node->left, callback)) {
        return 0;
    }
    if (!config_btree_preorder_traverse(node->right, callback)) {
        return 0;
    }
...
int config_btree_postorder_traverse(CONFIG_BTREE_NODE *node, CONFIG_BTREE_TRAVERSE_CB callback)
{
    ...
    if (!config_btree_postorder_traverse(node->left, callback)) {
        return 0;
    }
    if (!config_btree_postorder_traverse(node->right, callback)) {
        return 0;
    }
    if (!callback(node)) {
        return 0;
...
int parser_on_section_end(PARSER *parser)
{
    ...
    if (parser->status == PARSE_STATUS_COMMENT_LINE) {
        ...
        p = mem_trim((char*)(parser->str + parser->start1),
                    parser->pos - parser->start1, &len);
...
int parser_on_new_line(PARSER *parser)
{
    ...
    case PARSE_STATUS_GET_VALUE: {
        k = mem_trim((char*)(parser->str + parser->start1), parser->end1 - parser->start1, &klen);
        v = mem_trim((char*)(parser->str + parser->start2), parser->pos - parser->start2, &vlen);
        if (k == NULL) {
            return 0;
        }
        if (v == NULL) {
        ...
        if (!config_btree_insert_section_node(parser->config, parser->section_name, k, klen, v, vlen)) {
            return 0;
        }
        break;
    }
...
INI_CONFIG *ini_config_create_from_file(const char *filename, int isGBK)
{
    ...
    config = ini_config_create_from_string((unsigned char*)(buf), s.st_size, isGBK);
    free(buf);
...
int ini_config_set_string(INI_CONFIG *config, const char *section,
